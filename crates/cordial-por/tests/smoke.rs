...
fn stores_reputation_snapshot() {
    ...
    let mut state =
        ReputationState::new(0);


    state.set_reputation(
        node.clone(),
        42,
    );
...
fn exports_reputation_weights() {
    ...
    let weights =
        reputation_weights(&state);


    assert_eq!(
        weights.get(&validator),
        Some(&42)
    );
}
...
fn default_config_has_positive_initial_reputation() {
    ...
    assert!(
        config.initial_reputation > 0
    );


    assert!(
        config.initial_reputation <= config.scale
    );
}