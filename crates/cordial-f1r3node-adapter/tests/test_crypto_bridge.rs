...
use cordial_f1r3node_adapter::crypto_bridge::{
    ...
    Verifier, compute_block_hash,F1r3flyCryptoAdapter,
};

use cordial_miners_core::crypto::{hash_content as core_hash,CryptoVerifier};
use cordial_miners_core::types::{NodeId,BlockContent};
...
fn adapter_secp256k1_corrupted_signature_returns_err() {
    ...
    *signature.last_mut().unwrap()^=0xff; //flip last byte
    let creator = NodeId(pk);
    let adapter = F1r3flyCryptoAdapter::new(SigAlgorithm::Secp256k1);
    assert!(adapter.verify_block(&content,&signature,&creator).is_err());
}
...
fn adapter_secp256k1_forged_signature_returns_err() {
    ...
    let adapter = F1r3flyCryptoAdapter::new(SigAlgorithm::Secp256k1);
    assert!(adapter.verify_block(&content, &forged_signature, &creator).is_err());
}
...
fn adapter_secp256k1_tampered_content_returns_err() {
    ...
    let creator = NodeId(pk);
    let adapter = F1r3flyCryptoAdapter::new(SigAlgorithm::Secp256k1);
    assert!(adapter.verify_block(&tampered_content, &signature, &creator).is_err());
}
